...
CodePilotAI позволяет вести диалог с языковой моделью Google Gemini, используя файлы вашего проекта (из GitHub или локальной папки) в качестве контекста. Это крупное обновление, которое не только добавляет мощные новые функции, такие как **семантический поиск**, но и повышает общую стабильность и надежность приложения.

<a href="https://ibb.co/43wmKpN"><img src="https://i.ibb.co/qVLy7p1/Screenshot-29.png" alt="Screenshot-29" border="0"></a>
...
*   **Удобный интерфейс:**
    ...
    *   **Прогресс-бар анализа:** Наглядное отслеживание процесса анализа файлов.
    *   **Индикатор сети:** Показывает текущий статус подключения к интернету.
    *   **Пакетное управление контекстом:** Кнопка для быстрого исключения всех сообщений из контекста API.
...
*   **Семантический поиск:** `numpy` для векторных операций
...
4.  **Анализ:** Нажмите кнопку **"Анализировать"**. Прогресс будет отображаться на новом прогресс-баре под кнопками.
5.  **Диалог:** После завершения анализа (или без него в режиме простого чата) вы можете задавать вопросы. Например:
...
6.  **Сохранение:** Чтобы не анализировать проект каждый раз, сохраните сессию через меню **Файл -> Сохранить сессию**. В следующий раз вы сможете просто открыть ее.
